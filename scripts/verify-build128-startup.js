...
if (!reveal.includes('requireAuthIntroCompletion()')) errors.push('Global startup experience must require intro completion');
if (shell.includes('requireAuthIntroCompletion()') || shell.includes('markIntroNotRequired')) errors.push('AppShell must not compete with global intro ownership');
...
if (errors.length) {
  console.error('verify:build128-startup FAIL');
  errors.forEach((e) => console.error(`- ${e}`));
  process.exit(1);
}
console.log('verify:build128-startup PASS — one startup tree, gated services, monotonic trace');
